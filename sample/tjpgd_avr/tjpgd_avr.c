...
/ *** Modified for Microchip AVR microcontrollers (constant tables) ***
/----------------------------------------------------------------------------*/
...
static const uint8_t Zig[64] PROGMEM = {	/* Zigzag-order to raster-order conversion table */
...
static const uint16_t Ipsf[64] PROGMEM = {	/* See also aa_idct.png */
...
static const uint8_t Clip8[1024] PROGMEM = {
	/* 0..255 */
...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		for (i = 0; i < 64; i++) {				/* Load the table */
			zi = pgm_read_byte(&Zig[i]);		/* Zigzag-order to raster-order conversion */
			pb[zi] = (int32_t)((uint32_t)*data++ * pgm_read_word(&Ipsf[zi]));	/* Apply scale factor of Arai algorithm to the de-quantizers */
		}
	}
		...
		if (cmp && jd->ncomp != 3) {		/* Clear C blocks if not exist (monochrome image) */
			...
			do {
				...
				if (bc &= 0x0F) {					/* Bit length */
					...
					z = pgm_read_byte(&Zig[i]);		/* Zigzag-order to raster-order converted index */
