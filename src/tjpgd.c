...
)
{
	...
	for (blk = 0; blk < nby + 2; blk++) {	/* Get nby Y blocks and two C blocks */
		...
		if (cmp && jd->ncomp != 3) {		/* Clear C blocks if not exist (monochrome image) */
			...
			hd = jd->huffdata[id][0];
			d = huffext(jd, hb, hc, hc_len, hd);	/* Extract a huffman coded data (bit length) */
			...
			do {
				d = huffext(jd, hb, hc, hc_len, hd);		/* Extract a huffman coded value (zero runs and bit length) */
				if (d == 0) break;					/* EOB? */
